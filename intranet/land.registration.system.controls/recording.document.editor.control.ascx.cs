...
namespace Empiria.Web.UI.LRS {
  ...
  public partial class RecordingDocumentEditorControl : LRSDocumentEditorControl {

    protected override void ImplementsFillRecordingDocument(RecordingDocumentType documentType) {
      if (this.Recording.RecordingDocument.IsEmptyInstance) {
        this.Recording.RecordingDocument = RecordingDocument.Create(documentType, base.Recording);
      }
    ...
    private void LoadMainCombos() {
      ...
      ObjectList<Organization> offices = office.GetPropertyTitleOffices();
      HtmlSelectContent.LoadCombo(this.cboPropTitleIssueOffice, offices, "Id", "Alias", "( Seleccionar )", String.Empty, "No consta");

      GeneralList listType = GeneralList.Parse("PrivateContract.WitnessPosition.List");
      ObjectList<TypeAssociationInfo> positions = listType.GetTypeRelationItems<TypeAssociationInfo>();
      HtmlSelectContent.LoadCombo(this.cboPrivateDocMainWitnessPosition, positions, "Id", "DisplayName",
                                      "(Seleccionar)", String.Empty, "No consta");
    }
